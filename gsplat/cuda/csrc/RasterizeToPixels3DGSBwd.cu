...
////////////////////////////////////////////////////////////////
// Backward with explicit diff-style semantic output
////////////////////////////////////////////////////////////////

template <uint32_t SDIM, typename scalar_t>
__global__ void rasterize_to_pixels_3dgs_semantics_bwd_kernel(
    const uint32_t I,
    const uint32_t N,
    const uint32_t n_isects,
    const bool packed,
    ...
    const scalar_t *__restrict__ v_render_depths,
    const scalar_t *__restrict__ v_render_semantics,
    ...
    scalar_t *__restrict__ v_depths,
    scalar_t *__restrict__ v_semantics
) {
    auto block = cg::this_thread_block();
    uint32_t image_id = block.group_index().x;
    uint32_t tile_id =
    ...
    render_alphas += image_id * image_height * image_width;
    last_ids += image_id * image_height * image_width;
    v_render_colors += image_id * image_height * image_width * 3;
    v_render_alphas += image_id * image_height * image_width;
    v_render_depths += image_id * image_height * image_width;
    v_render_semantics += image_id * image_height * image_width * SDIM;
    if (backgrounds != nullptr) {
    ...
    float *colors_batch = (float *)&conic_batch[block_size];
    float *depths_batch = colors_batch + block_size * 3;
    float *semantics_batch = depths_batch + block_size;

    float T_final = 1.0f - render_alphas[pix_id];
    float T = T_final;
    float color_buffer[3] = {0.f, 0.f, 0.f};
    float depth_buffer = 0.f;
    float semantic_buffer[SDIM] = {0.f};
    const int32_t bin_final = inside ? last_ids[pix_id] : 0;

    float v_render_c[3];
#pragma unroll
    for (uint32_t k = 0; k < 3; ++k) {
        v_render_c[k] = v_render_colors[pix_id * 3 + k];
    }
    const float v_render_a = v_render_alphas[pix_id];
    const float v_render_d = v_render_depths[pix_id];
    float v_render_s[SDIM];
#pragma unroll
    for (uint32_t k = 0; k < SDIM; ++k) {
        v_render_s[k] = v_render_semantics[pix_id * SDIM + k];
    }
...
#pragma unroll
            for (uint32_t k = 0; k < SDIM; ++k) {
                semantics_batch[tr * SDIM + k] = semantics[g * SDIM + k];
            }
        }
        ...
        for (uint32_t t = max(0, batch_end - warp_bin_final); t < batch_size;
            ...
            float v_semantic_local[SDIM] = {0.f};
            vec3 v_conic_local = {0.f, 0.f, 0.f};
            vec2 v_xy_local = {0.f, 0.f};
            vec2 v_xy_abs_local = {0.f, 0.f};
            float v_opacity_local = 0.f;
...
#pragma unroll
                for (uint32_t k = 0; k < 3; ++k) {
                    v_color_local[k] = fac * v_render_c[k];
                }
                v_depth_local = fac * v_render_d;
#pragma unroll
                for (uint32_t k = 0; k < SDIM; ++k) {
                    v_semantic_local[k] = fac * v_render_s[k];
                }
...
#pragma unroll
                for (uint32_t k = 0; k < 3; ++k) {
                    v_alpha +=
                        (colors_batch[t * 3 + k] * T - color_buffer[k] * ra) *
                        v_render_c[k];
                }
                v_alpha +=
                    (depths_batch[t] * T - depth_buffer * ra) * v_render_d;
#pragma unroll
                for (uint32_t k = 0; k < SDIM; ++k) {
                    v_alpha +=
                        (semantics_batch[t * SDIM + k] * T -
                         semantic_buffer[k] * ra) *
                        v_render_s[k];
                }
...
#pragma unroll
                for (uint32_t k = 0; k < 3; ++k) {
                    color_buffer[k] += colors_batch[t * 3 + k] * fac;
                }
                depth_buffer += depths_batch[t] * fac;
#pragma unroll
                for (uint32_t k = 0; k < SDIM; ++k) {
                    semantic_buffer[k] +=
                        semantics_batch[t * SDIM + k] * fac;
                }
            }

            warpSum<3>(v_color_local, warp);
            warpSum(v_depth_local, warp);
            warpSum<SDIM>(v_semantic_local, warp);
            warpSum(v_conic_local, warp);
            warpSum(v_xy_local, warp);
            if (v_means2d_abs != nullptr) {
                warpSum(v_xy_abs_local, warp);
            ...
            if (warp.thread_rank() == 0) {
                ...
                float *v_color_ptr = (float *)(v_colors) + 3 * g;
#pragma unroll
                for (uint32_t k = 0; k < 3; ++k) {
                    gpuAtomicAdd(v_color_ptr + k, v_color_local[k]);
                }

                gpuAtomicAdd(v_depths + g, v_depth_local);

                float *v_semantic_ptr = (float *)(v_semantics) + SDIM * g;
#pragma unroll
                for (uint32_t k = 0; k < SDIM; ++k) {
                    gpuAtomicAdd(v_semantic_ptr + k, v_semantic_local[k]);
                }
...
void launch_rasterize_to_pixels_3dgs_semantics_bwd_kernel(
    ...
    const at::Tensor v_render_semantics,
    at::optional<at::Tensor> v_means2d_abs,
    at::Tensor v_means2d,
    at::Tensor v_conics,
    at::Tensor v_colors,
    ...
    if (cudaFuncSetAttribute(
            rasterize_to_pixels_3dgs_semantics_bwd_kernel<SDIM, float>,
            cudaFuncAttributeMaxDynamicSharedMemorySize,
            shmem_size
        ) != cudaSuccess) {
        AT_ERROR(
    ...
    rasterize_to_pixels_3dgs_semantics_bwd_kernel<SDIM, float>
        <<<grid, threads, shmem_size, at::cuda::getCurrentCUDAStream()>>>(
            ...
            v_render_semantics.data_ptr<float>(),
            v_means2d_abs.has_value()
                ? reinterpret_cast<vec2 *>(
                      v_means2d_abs.value().data_ptr<float>()
                  )
            ...
            v_semantics.data_ptr<float>()
        );
}

#define __INS_SEM__(SDIM)                                                       \
    template void launch_rasterize_to_pixels_3dgs_semantics_bwd_kernel<SDIM>(   \
        const at::Tensor means2d,                                               \
        const at::Tensor conics,                                                \
        const at::Tensor colors,                                                \
        const at::Tensor opacities,                                             \
